...
int main(int argc, char* const argv[])
{
            ...
            if (optind+1 < argc) {
                ...
                if (service != NULL && ifName.size() > 0) {
                    ...
                    while (optind < argc) {
                        ...
                        } else if (strcmp(argv[optind], "cstr") == 0) {
                            optind++;
                            if (optind >= argc) {
                                aerr << "service: no string supplied for 'cstr'" << endl;
                                wantsUsage = true;
                                result = 10;
                                break;
                            }
                            data.writeCString((char *)argv[optind++]);
    ...
    if (wantsUsage) {
        aout << "Usage: service [-h|-?]\n"
                ...
                "   cstr: Write the C-style string STR into the send parcel.\n";
